import { useState } from "react";

interface Breadcumb10Props {
  path: string[];
}

const Breadcumb10: React.FC<Breadcumb10Props> = ({ path }) => {
  const [widgetState, setWidgetState] = useState({
    shareToggle: false,
    saveToggle: false,
  });

  const toggleShare = () => {
    setWidgetState((prevState) => ({
      ...prevState,
      shareToggle: !prevState.shareToggle,
    }));
  };

  const toggleSave = () => {
    setWidgetState((prevState) => ({
      ...prevState,
      saveToggle: !prevState.saveToggle,
    }));
  };

  return (
    <section className="breadcumb-section">
      <div className="container">
        <div className="row">
          <div className="col-sm-8 col-lg-10">
            <div className="breadcumb-style1 mb10-xs">
              <div className="breadcumb-list">
                {path?.map((item, i) => (
                  <a key={i}>{item}</a>
                ))}
              </div>
            </div>
          </div>
          <div className="col-sm-4 col-lg-2">
            <div className="d-flex align-items-center justify-content-sm-end">
              <a
                onClick={toggleShare}
                className="position-relative ui-share-toggle"
                aria-expanded={widgetState.shareToggle}
                aria-haspopup="true"
                role="button" // Add role attribute for accessibility
              >
                <div
                  className={`share-save-widget d-flex align-items-center ${
                    widgetState.shareToggle ? "active" : ""
                  }`}
                >
                  <span className="icon flaticon-share dark-color fz12 mr10" />
                  <div className="h6 mb-0">Share</div>
                </div>
                {widgetState.shareToggle && (
                  <div className="ui-social-media">
                    <a>
                      <i className="fab fa-facebook-f"></i>
                    </a>
                    <a>
                      <i className="fab fa-twitter"></i>
                    </a>
                    <a>
                      <i className="fab fa-linkedin-in"></i>
                    </a>
                    <a>
                      <i className="fab fa-pinterest-p"></i>
                    </a>
                  </div>
                )}
              </a>
              <a onClick={toggleSave} role="button"> {/* Add role attribute for accessibility */}
                <div
                  className={`share-save-widget d-flex align-items-center ml15 ${
                    widgetState.saveToggle ? "active" : ""
                  }`}
                >
                  <span className="icon flaticon-like dark-color fz12 mr10" />
                  <div className="h6 mb-0">Save</div>
                </div>
              </a>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Breadcumb10;
